...
# Climate, soil and nutrient addition
# PCA analysis for soil
df.pca <- df %>% 
  select(pH, SM, STC, STN, STP)
res.pca <- prcomp(df.pca, scale = TRUE)
summary(res.pca)
soil <- data.frame(res.pca$x[, 1:2])  # soil data
climate <- df %>% 
  select(MAT, MAP)  # cliamte data
treat <- df %>% 
  select(treat)  %>% 
  data.frame()  # treatment
# general linear models
pdf("./outputs/varPart.pdf", height = 5.5, width = 10.5)
par(mfrow = c(1, 2))
# by vector length
A <- summary(lm(df$vecL ~ climate$MAT + climate$MAP))$r.squared
B <- summary(lm(df$vecL ~ soil$PC1 + soil$PC2))$r.squared
C <- summary(lm(df$vecL ~ treat$treat))$r.squared
AB <- summary(lm(df$vecL ~ climate$MAT + climate$MAP + 
                   soil$PC1 + soil$PC2))$r.squared
AC <- summary(lm(df$vecL ~ climate$MAT + climate$MAP + treat$treat))$r.squared
BC <- summary(lm(df$vecL ~ soil$PC1 + soil$PC2 + treat$treat))$r.squared
ABC <- summary(lm(df$vecL ~ climate$MAT + soil$PC1 + soil$PC2 + 
                    climate$MAP + treat$treat))$r.squared
varPart(A, B, C, AB, AC, BC, ABC,
        A.name = "Climate", B.name = "Soil", C.name = "N addition")
# by vector angle
A <- summary(lm(df$vecA ~ climate$MAT + climate$MAP))$r.squared
B <- summary(lm(df$vecA ~ soil$PC1 + soil$PC2))$r.squared
C <- summary(lm(df$vecA ~ treat$treat))$r.squared
AB <- summary(lm(df$vecA ~ climate$MAT + climate$MAP + 
                   soil$PC1 + soil$PC2))$r.squared
AC <- summary(lm(df$vecA ~ climate$MAT + climate$MAP + treat$treat))$r.squared
BC <- summary(lm(df$vecA ~ soil$PC1 + soil$PC2 + treat$treat))$r.squared
ABC <- summary(lm(df$vecA ~ climate$MAT + soil$PC1 + soil$PC2 + 
                    climate$MAP + treat$treat))$r.squared
varPart(A, B, C, AB, AC, BC, ABC,
        A.name = "Climate", B.name = "Soil", C.name = "N addition")
dev.off()
...
# interactions between climate, soil and N addition ---
# by vector length
modA <- lmer(vecL ~ MAT * treat + (1|site2/fb/fp), data = df)
anova(modA, type = "I", ddf = "Kenward-Roger")
modB <- lmer(vecL ~ MAP * treat + (1|site2/fb/fp), data = df)
anova(modB, type = "I", ddf = "Kenward-Roger")
# by vector angle
modA <- lmer(vecA ~ MAT * treat + (1|site2/fb/fp), data = df)
anova(modA, type = "I", ddf = "Kenward-Roger")
modB <- lmer(vecA ~ MAP * treat + (1|site2/fb/fp), data = df)
anova(modB, type = "I", ddf = "Kenward-Roger")
